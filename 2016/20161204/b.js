...
inp.forEach(function(line){
    ...
    if (line && parsedLine.calcCheckSum === parsedLine.checkSum) {
        parsedLine.roomName = getRoomName(parsedLine).trim();
        if (parsedLine.roomName.indexOf("north") > -1) console.log(parsedLine);
        // console.log(parsedLine.roomName);
    }
...
function getRoomName(parsedLine){
    var name = "";
    var letters = "abcdefghijklmnopqrstuvwxyz";
    for (var i = 0; i < parsedLine.name.length; i++){
        if (parsedLine.name[i] === "-") name += " "
        else {
            name += letters[(letters.indexOf(parsedLine.name[i])+Number(parsedLine.sectorId))%26];
        }
    }
    return name;
}
